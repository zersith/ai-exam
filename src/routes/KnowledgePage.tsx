...
const knowledgeBase: KnowledgeSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        term: 'AI Skill（技能）是什么',
        content: 'Skill 是将特定工作流程封装为可复用的 AI 指令包，包含角色设定、执行步骤、输出格式和工具调用。它让 AI 从"通用助手"变成"专项专家"。PM 的价值在于：识别团队中高频、标准化的工作流程，将其抽象为 Skill，让团队成员一键调用。例如：竞品分析 Skill = 自动收集信息 → 按维度对比 → 输出结构化报告。',
        tags: ['Skill', '概念'],
      },
      {
        term: 'Skill 的设计方法论',
        content: '设计 Skill 四步法：①识别高频场景（团队每周至少做 3 次的任务）→ ②拆解任务步骤（自己手动做一遍，记录每一步的输入和输出）→ ③编写 Skill 指令（角色定义 + 步骤分解 + 输出模板 + 边界约束）→ ④团队测试迭代（至少 3 个人试用并反馈）。好的 Skill 让新手也能产出接近老手质量的结果。',
        tags: ['Skill', '设计'],
      },
      {
        term: 'Skill 的沉淀与团队推广',
        content: '将验证有效的 Skill 存入团队共享知识库（飞书/语泉/Notion），附上：Skill 名称和一句话描述、适用场景和前置条件、使用示例（输入→输出）、已知局限和注意事项。定期（每月）评审 Skill 库：淘汰不再适用的、更新效果下降的、补充新场景的。Skill 库是团队 AI 能力的核心资产。',
        tags: ['Skill', '团队'],
      },
      {
        term: '从 Prompt 到 Skill 的进化路径',
        content: '单次 Prompt（解决一次性问题）→ Prompt 模板（同类问题可复用，需手动替换变量）→ Skill（封装完整工作流，包含多步推理和工具调用）。PM 的成长路径：先写好单次 Prompt → 识别可模板化的场景 → 逐步封装为 Skill。不是所有 Prompt 都需要变成 Skill，只封装那些高频、多步骤、有质量要求的场景。',
        tags: ['Skill', '进阶'],
      },
    ],
  },
